...
func enviarEstadoDigimon(client pb.ServicioRegionalesClient, nombre, tipo string, sacrificado bool) {
	...
	if sacrificado {
		log.Printf("[Isla File] Estado enviado: %s SACRIFICADO", nombre)
	} else {
		log.Printf("[Isla File] Estado enviado: %s NO-SACRIFICADO", nombre)
	}
}
...
func consultarEstadoTerminacion(client pb.ServicioRegionalesClient) bool {
	...
	if err != nil {
		log.Printf("[Isla File] Error al consultar estado de terminación: %v", err)
		return false
	}

	if res.Activo {
		log.Println("[Isla File] Señal de terminación recibida. Finalizando ejecución.")
		return true
	}
	return false
}
...
func ejecutarCiclo(client pb.ServicioRegionalesClient, s *servidorRegional, digimons [][2]string) {
    ...
    for {
        ...
        if consultarEstadoTerminacion(client) {
            log.Println("[Isla File] Finalizando ejecucion...")
            break
        }
...
func main() {
	...
	log.Println("[Isla File] Esperando a que Diaboromon esté activo para empezar a enviar datos de Digimon...")
	for {
		...
		if err != nil {
			log.Printf("[Isla File] Error al consultar el estado de Diaboromon: %v", err)
		}

		// Verificar si Diaboromon está activo
		if res.Activo {
			log.Println("[Isla File] Diaboromon está activo. Comenzando a enviar datos.")
			break
		}
	}
