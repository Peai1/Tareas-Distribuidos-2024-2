...
func (s *Server) Command(ctx context.Context, msg *messages.Cmd) (*messages.VectorClock, error) {
	log.Printf("[%s] Comando recibido del Supervisor: %s %s %s", serverName, msg.Cmd, msg.Sector, msg.Base)
	...
	log.Printf("[%s] Ejecutado exitosamente: Region[%s] VectorClock[%v %v %v]", serverName, msg.Sector, response.X, response.Y, response.Z)
...
func (s *Server) Soldados(ctx context.Context, msg *messages.Info) (*messages.ValueInfo, error) {
	log.Printf("[%s] Solicitud recibida del Broker: Region[%s] Producto[%s]", serverName, msg.Sector, msg.Base)

	vc, exists := fileVectorClock[msg.Sector]
	if !exists {
		log.Printf("[%s] Dicha region no existe aquí", serverName)
	...
	if !exists {
		log.Printf("[%s] Dicho producto no existe en %s aqui", serverName, msg.Sector)
		return &messages.ValueInfo{Valor: -1}, nil
	}
...
func (s *Server) RequestMerge(ctx context.Context, msg *messages.AskMerge) (*messages.Ack, error) {
	log.Printf("[%s] Ejecutando el merge solicitado...", serverName)
	executeMerge()
	log.Printf("[%s] Merge completado...", serverName)
	return &messages.Ack{Value: 1}, nil
}

func (s *Server) Merge(ctx context.Context, msg *messages.AskMerge) (*messages.AllInfo, error) {
	log.Printf("[%s] Solicitud de merge recibida desde %s, enviando logs...", serverName, msg.FulcrumName)
...
func (s *Server) PropagateChanges(ctx context.Context, msg *messages.CommandsAndVectorClocks) (*messages.Ack, error) {
	log.Printf("[%s] Cambios recibidos, realizando el merge...", serverName)
...
func logAndVectorClocks() *messages.AllInfo {
	...
	for scanner.Scan() {
		...
		cmd.Cmd = parts[1]              // Comando AgregarProducto, RenombrarProducto, ActualizarValor, BorrarProducto
		...
		case "ActualizarValor":
			val, _ := strconv.Atoi(parts[4])
			cmd.Valor = int32(val)
		//case "BorrarProducto": // Caso no necesario de ser usado
		default:
			// Hacer nada
		}
...
func main() {
	...
	os.Mkdir("Servidor1", 0777)
	os.Mkdir("Servidor2", 0777)
	os.Mkdir("Servidor3", 0777)

	log.Printf("[%s] Iniciando en puerto %s...", serverName, servePort)
